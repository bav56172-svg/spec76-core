...
export function analyzeRequestLocally(request: Request): RequestAnalysisDraft {
  ...
  const areaMatch = text.match(
    /(\d+(?:[.,]\d+)?)\s*(?:м2|м²|кв(?:адратных)?\s*метр)/i,
  );
  const volumeMatch = text.match(
    /(\d+(?:[.,]\d+)?)\s*(?:м3|м³|куб(?:ов|ических\s*метр)?)/i,
  );
  ...
  if (services.length === 0) {
    ...
    clarifications.push(
      "Какой результат должен быть получен после выполнения работ?",
    );
  }
  ...
  const confidence = Math.min(
    ...
    0.45 +
      ...
      equipment.length * 0.07 +
      (estimatedScope ? 0.12 : 0),
  );
...
export async function getCurrentRequestAnalysis(
  requestId: string,
): Promise<ServiceResult<RequestAnalysis | null>> {
  ...
  if (error) {
    return databaseFailure(error, "Не удалось получить анализ заявки.");
  }
...
export async function runRequestAnalysis(
  request: Request,
): Promise<ServiceResult<RequestAnalysis>> {
  ...
  if (resetError) {
    return databaseFailure(
      resetError,
      "Не удалось обновить предыдущий анализ заявки.",
    );
  }
  ...
  if (error) {
    return databaseFailure(error, "Не удалось сохранить анализ заявки.");
  }
