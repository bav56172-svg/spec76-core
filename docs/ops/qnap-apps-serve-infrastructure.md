...
### 3.19 Бэкапы БД (spec76-core + eceupo) на существующий бэкап-контур NAS — ЗАВЕРШЕНО

**Решение пользователя** (на NAS уже настроен свой бэкап-контур: Hybrid Backup Sync 3 с заданиями Storage/Public→Dev/Backup и Storage/Public→SeagateBackup, USB-диск на ~1.82ТБ):
1. Куда писать бэкапы БД: в существующую сетевую шару NAS — чтобы уже настроенные задания HBS3 подхватывали дампы автоматически.
2. Расписание/хранение: раз в сутки ночью (03:00 по Москве), хранить 7 последних копий.

**Попытка через NFS не удалась** (`showmount -e 192.168.1.164` → `clnt_create: RPC: Unable to receive`, NFS-сервис на NAS не включён) → **пивот на SMB/CIFS**, т.к. именно его используют HBS3/Time Machine на этом NAS.

**Учётная запись на NAS:** создан отдельный пользователь `svc-backup` (не личный `avbalashov`) через QTS (Control Panel → Users → Create), с правом **Чтение/Запись только на шару Public** (Control Panel → Shared Folders → Public → Permission). Создание пользователя и ввод пароля в форму — сделал пользователь лично через веб-интерфейс QTS (я не выполняю это сам — создание учётных записей и ввод паролей вне моих допустимых действий в любом виде, даже по SSH/API).

**Монтирование на apps-serve:**
- Файл учётных данных `/etc/samba-credentials-nas` (`username=svc-backup`, `password=...`), права `600`, `root:root`, не в git.
- Точка монтирования: `/mnt/nas-backup` → `//192.168.1.164/Public` (protocol SMB3, `vers=3.0`).
- Запись в `/etc/fstab`:
  ```
  //192.168.1.164/Public /mnt/nas-backup cifs credentials=/etc/samba-credentials-nas,uid=agent,gid=agent,vers=3.0,file_mode=0600,dir_mode=0700,_netdev,x-systemd.automount,noauto 0 0
  ```
- Проверено: `mount /mnt/nas-backup` работает (в т.ч. через fstab-запись), доступно 3.2TB свободно из 4TB шары, запись/чтение подтверждены.
- Бэкапы пишутся в подпапку `/mnt/nas-backup/db-backups/` (создана автоматически).

**Скрипт бэкапа** — `/home/agent/scripts/db-backup.sh` (владелец `agent`, права `700`, т.к. содержит пароли БД в открытом виде):
- Дамп **spec76-core**: `docker exec -e PGPASSWORD=... supabase-db pg_dump -U postgres -d postgres --no-owner | gzip` — через `docker exec` внутрь контейнера, т.к. `supabase-db` не публикует порт на хост (только `supabase-pooler`/Supavisor, который требует tenant-идентификатор, см. 3.16) — pg_dump версии 17.6 из самого образа.
- Дамп **eceupo**: `docker exec -e PGPASSWORD=... eceupo-postgres pg_dump -U eceupo -d eceupo --no-owner | gzip` — pg_dump версии 16.15 из образа, БД слушает host-порт 5433, но exec тоже используется для консистентности подхода.
- Перед записью скрипт проверяет `mountpoint -q /mnt/nas-backup` — если шара не смонтирована (NAS недоступен), бэкап прерывается с ошибкой в лог, **ничего не пишется локально** (чтобы не потерять бэкап "в никуда").
- Ротация — **по количеству файлов, не по mtime**: хранятся последние 7 файлов на каждый проект (`ls -1t ... | tail -n +8 | rm`) — устойчиво к пропущенной ночи (в отличие от `find -mtime +7`, где пропуск не восстановит нужное окно).
- Лог: `/var/log/db-backup.log` (создан, `chown agent:agent`).

**Расписание:** `cron` пользователя `agent`, `0 0 * * *` — важно: **VM работает в UTC** (`Etc/UTC`), поэтому `0 0` в crontab = 00:00 UTC = **03:00 по Москве** (UTC+3). Если сервер когда-либо переведут на другой часовой пояс — эту строчку нужно будет пересчитать.

**Тест:** ручной запуск скрипта прошёл успешно — `spec76-core_20260912_140330.sql.gz` (57K) и `eceupo_20260912_140330.sql.gz` (3.0K) записаны на `/mnt/nas-backup/db-backups/`, лог подтверждает `OK` для обеих баз. Дальше эти же файлы попадут в существующие HBS3-задания (Public → Dev/Backup, Public → SeagateBackup) по их собственному еженедельному расписанию (Sunday 02:00) — это отдельный, уже существующий слой резервирования поверх ежесуточных дампов.

**НЕ является инфраструктурной проблемой eceupo/spec76-core:** apps-serve-специфичная настройка (файл `/etc/fstab`, cron, скрипт, файл учётных данных) — в репозиториях проектов ничего не меняется.
...
- [x] Бэкапы БД (spec76-core + eceupo) на бэкап-контур NAS — готово: NAS-шара Public смонтирована по SMB через отдельную учётку `svc-backup`, скрипт `pg_dump` + ежесуточный cron (03:00 МСК) + ротация на 7 копий, протестировано вручную, дампы автоматически подхватываются существующими заданиями HBS3 (см. 3.19)
