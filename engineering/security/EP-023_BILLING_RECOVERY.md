...
**Версия:** 0.7
**Статус:** Approved Recovery Boundary (утверждённая граница восстановления)
**Владелец:** Platform Owner (владелец платформы)
**Release:** 0.4
**Engineering Package:** `EP-023`
**Ветка:** `engineering/ep-023-security-recovery`
**Следующий пересмотр:** при проектировании отдельной Capability пользовательского платёжного сценария
...
## Граница утверждения Platform Owner

Platform Owner 2026-07-22 утвердил границы Billing Recovery: безопасную миграцию, RLS, серверный webhook, Idempotency и проверенный rollback.

До допуска полного пользовательского платёжного сценария в production необходимо:

1. отдельно определить Capability (возможность платформы — законченный функциональный контур) для безопасного создания связи `user_id` и `stripe_customer_id`;
2. реализовать серверное создание Stripe Customer и Checkout Session;
3. проверить успешную синхронизацию заранее связанной подписки;
4. провести отдельный Security и Architecture Gate для финансового пользовательского сценария.
...
Зафиксировать отдельную Capability для создания связи пользователя со Stripe Customer, Checkout Session и успешного пользовательского платёжного сценария. До её архитектурного и безопасностного допуска текущий Billing Recovery не расширять.
